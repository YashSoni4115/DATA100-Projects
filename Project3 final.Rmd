...
```{r error=TRUE, tags=c()}
(cyclones_raw <- cyclone_files |> map(read_cyclone)
 )
```
```{r error=TRUE}
...
```{r error=TRUE, tags=c()}
(cyclones_data <- cyclones_raw |>   map(convert_latlong) |> list_rbind()
)
```
...
(cyclones <- cyclones_data |> mutate(observ_time = make_datetime(ObservYear, Month, Day, Hour, Minute),
      category = ordered(
        case_when(
          max_wind <= 33 ~ "TD",
          max_wind <= 63 ~ "TS",
...
```{r error=TRUE, tags=c()}
(basins <- cyclones |> summarize(.by = Basin, count = n(),year_start = min(ObservYear, na.rm = TRUE),
...
```{r error=TRUE, tags=c()}
(status <- cyclones |>    summarize(.by = c(category, status),count = n(),first_year = min(ObservYear, na.rm = TRUE), last_year = max(ObservYear, na.rm = TRUE))
)
```
...
(is_rapid_intensifying <- cyclones |> mutate(.by = c(Basin, Number, NameYear), six_hour_incr = max_wind - lag(max_wind, 1), twelve_hour_incr = max_wind - lag(max_wind, 2), eighteen_hour_incr = max_wind - lag(max_wind, 3), one_day_incr = max_wind - lag(max_wind, 4),
    ...
    filter(if_any(ends_with("incr"), ~ . >= rapid)) |>
    relocate(six_hour_incr:forward_one_day_incr)
)
```
...
```{r error=TRUE, tags=c()}
(rapid_intensifying_cyclones <- cyclones |> semi_join(is_rapid_intensifying, join_by(Basin, Number, NameYear)) |> relocate(max_wind, category)
)
```
...
```{r error=TRUE, tags=c()}
(rapid_intensifying_plot <- rapid_intensifying_cyclones |>    summarize(.by = c(Basin, NameYear, Number), max_cat = max(category)) |> ggplot(aes(x = NameYear, fill = max_cat)) + geom_bar() + facet_wrap(~Basin) +
   coord_flip() +
   labs(
...
ymd_extract <- function(ref_years, times) {for (i in 1:length(times)) {year(times[i]) <- year(times[i]) - ref_years[i]
  }
  times
}
```
...
```{r error=TRUE, tags=c()}
(seasons <- cyclones |> mutate(.by = c(Basin, Number, NameYear), max_cat = max(category, na.rm = TRUE)) |> filter(max_cat >= "1") |> 
   summarize(.by = c(NameYear, Basin), season_begin = ymd_extract(NameYear, min(observ_time)),
     season_end = ymd_extract(NameYear, max(observ_time)))
)
```
...
```{r error=TRUE, tags=c()}
(seasons_segment_plot <- seasons |>    ggplot(aes(x = season_begin, y = NameYear, color = Basin)) +
   geom_point(size = 0.5) +
...
```{r error=TRUE, tags=c()}
(seasons_length_plot  <- seasons |>    mutate(length_days = (season_begin %--% season_end) / days(1)) |>
   ggplot(aes(x = NameYear, y = length_days, color = Basin)) + geom_line() + geom_smooth(color = "orange") + facet_wrap(~Basin) +
   labs(title = "Storms that grow to hurricanes are seen during longer seasons in a year",
     subtitle = "The growing trend looks more steady in the Atlantic basin", x = "Year", y = "Length of the hurricane seasons", caption = "Data from NOAA")
)
```

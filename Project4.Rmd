...
read_cyclone <- function(single_file = AT_cyclone) {
  output <- str_c(cyclone_data_address, single_file, sep = "") |>
    read_csv(col_names = as.character(1:4)) |>
    ...
    mutate (
     BasinNumberYear = if_else((if_all(status:r_max_wind, is.na)), `1`, NA),
     Name = if_else((if_all(status:r_max_wind, is.na)), `2`, NA),
     Entries = if_else((if_all(status:r_max_wind, is.na)), `3`, NA),
   ) |>
   ...
   separate_wider_position(
     cols = `2`,
     width = c(Hour = 2, Minute = 2)
   ) |>
   rename (
     Identifier = `3`
   ) |>
   mutate(
     across(all_of(c('Number', 'NameYear', 'ObservYear', 'Month', 'Day', 'Hour', 'Minute')), as.integer),
     across(all_of(c("max_wind", "min_pressure", "NE_extend_34", "SE_extend_34", "SW_extend_34", "NW_extend_34", "NE_extend_50", "SE_extend_50", "SW_extend_50", "NW_extend_50", "NE_extend_64", "SE_extend_64", "SW_extend_64", "NW_extend_64", "r_max_wind"
)),parse_double)
) |>
   mutate(
...
convert_latlong <- function(df) {
  output <- df |>
    mutate(
     num_lat = ifelse(str_sub(latitude, -1,-1) == 'N', sub("[^:alpha]$", '', latitude), str_c('-', sub("[^:alpha]$", '', latitude))),
     num_long = ifelse(str_sub(longitude, -1,-1) == 'E', sub("[^:alpha]$", '', longitude), str_c('-', sub("[^:alpha]$", '', longitude))),
     num_lat = ifelse(str_sub(num_lat, 1,2) == '--', sub("^[--]", '', num_lat), num_lat),
     num_long = ifelse(str_sub(num_long, 1,2) == '--', sub("^[--]", '', num_long), num_long),
     across(c(num_lat, num_long), as.numeric)
    ) 
  output
}
...
(cyclones <- cyclone_files |>
    ...
    purrr::map(convert_latlong) |>
    list_rbind() |>
    mutate(
      observ_time = make_datetime(year = ObservYear, month = Month, day = Day, hour = Hour, min = Minute),
      category = ordered(
        case_when(
          max_wind >= 137 ~ '5',
          max_wind >= 113 ~ '4',
...
```{r error=TRUE, tags=c()}
(atlantic_cyclones_cat_count <- cyclones_cat_count %>% filter(Basin == "AL")
)
(pacific_cyclones_cat_count <- cyclones_cat_count %>% filter(Basin != "AL")
)
```
```{r error=TRUE}
...
(count_plot <- joined_count_by_cat |>
  ...
  geom_jitter() +
  labs(x = "Number of hurricanes in the Atlantic", y = "Number of hurricanes in the Pacific", title = "Numbers of hurricanes in a year, Atlantic vs Pacific", subtitle = "Very slight negative relation", caption = "Data from NOAA")
)
```
```{r error=TRUE}

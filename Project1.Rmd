...
|------------|------------|-----------|---------------------|---------------|
|169066745   |Anas        |Eldesouky  |elde6745@mylaurier.ca|      20%      |
|212299900   |Nithin      |Kumar      |gopa9900@mylaurier.ca|      20%      |
|169060597   |Yash        |Soni       |soni0597@mylaurier.ca|      20%      |
|169067604   |Pranathi    |Namuduri   |namu7604@mylaurier.ca|      20%      |
|169025016   |Nathen      |Wilk       |wilk5016@mylaurier.ca|      20%      |
|            |            |           |                     |               |
...
```{r error=TRUE, tags=c()}
(storm_rough <- storms |>
  summarise(start_year = min(year), end_year = max(year), count = n_distinct(year, name))
)
```
```{r error=TRUE}
...
(storm_range <- storms |>
  group_by(year) |>
   summarise(year = unique(year), min_wind = min(wind), max_wind = max(wind), min_lat = min(lat), max_lat = max(lat))
)
```
```{r error=TRUE}
...
(cat_status_plot <- storms |>
   ...
   geom_count(mapping = aes(x = category, y = status))
)
```
```{r error=TRUE}
...
(status_wind_plot <- storms |>
  ggplot(mapping = aes(x = wind, y = status)) + 
    geom_jitter(mapping = aes(x = wind, y = status), alpha = 0.2) + status_wind_labels
)
```
```{r error=TRUE}
...
(label_emily_plot <- storms |>
  ...
  ggplot(mapping = aes(x = wind, y = status)) +
    geom_jitter(mapping = aes(x = wind, y = status), alpha = 0.2) +
  geom_label_repel(
    data = wrong_status, aes(label = name)
    ) +
  geom_point(
    data = emily_2005,
    color = "red"
...
(replot_no_emily_plot <- storms |>
    ...
    ggplot(mapping = aes(x = wind, y = status)) +
    geom_jitter(mapping = aes(x = wind, y = status), alpha = 0.2) + replot_no_emily_label
)
```
```{r error=TRUE}
...
(correct_emily_plot <- new2_storms |>
  ...
  ggplot(mapping = aes(x = wind, y = status)) + 
    geom_jitter(mapping = aes(x = wind, y = status), alpha = 0.2) +
    geom_label_repel(data = correct_emily, aes(label = name)) +
...
(ridges_plot <- new2_storms |>
  ggplot(mapping = aes(
  x = wind, y = status, fill = status, color = status
    )
  ) +
  geom_density_ridges(
  mapping = aes(x = wind, y = status, fill = status, color = status),
...
(category_jitter_plot <- new2_storms |> 
    ...
    geom_jitter(mapping = aes(x = wind, y = factor_cat, color = factor_cat)) +
  theme(legend.position = "bottom") +
  guides(
    color = guide_legend(title = "Category", nrow = 1)) + category_plot_labels
)

(category_ridges_plot <- new2_storms |>
    filter(status == "hurricane") |>
  ggplot(mapping = aes(x = wind, y = factor_cat, color = factor_cat, fill = factor_cat)) +
    geom_density_ridges(mapping = aes(x = wind, y = factor_cat, color = factor_cat, fill = factor_cat), show.legend = FALSE, alpha = 0.5) + category_plot_labels
)
...
(new2_storms_summary <- new2_storms |>
  group_by(category) |>
   summarise(category = unique(category), min_wind = min(wind), max_wind = max(wind))
)
```
```{r error=TRUE}
...
(pressure_box_plot <- new2_storms |>
  ...
  ) + geom_boxplot(mapping = aes(x = pressure, y = factor_cat)) +
  pressure_plot_labels +
...
(pressure_ridges_plot <- new2_storms |>
  ...
  geom_density_ridges(mapping = aes(x = pressure, y = factor_cat), alpha = 0.5) +
  pressure_plot_labels +
  pressure_plot_x_scale
)
...
(storm_count_plot <- storms_by_storm |>
 ...
 geom_bar(mapping = aes(x = year), color = "black") +
...
(storms_in_year_plot <- storms_by_storm |>
  ...
  |> ggplot(
    mapping = aes(y = name, color = factor_max_cat)
    ) +                                 
  geom_point(mapping = aes(x = first_day, y = name)) +
    geom_segment(aes(x = first_day, y = name, xend = last_day, yend = name)) +
  scale_x_date(
    name = NULL,
    date_labels = "%b"
    ) +                                 
...
(duration_vs_category_bar_plot <- storms_by_storm |>
  ...
  geom_bar(mapping = aes(x = days), color = "black") +
  theme(legend.position = "bottom") +            
  guides(
    fill = guide_legend(
...
(box_ridges_combined_plot <- storms_by_storm |>
  ggplot(
    mapping = aes(x = days, y = factor_max_cat)
  ) + 
    geom_boxplot(mapping = aes(x = days, y = factor_max_cat)) + 
    geom_density_ridges(mapping = aes(x = days, y = factor_max_cat, fill = factor_max_cat), color = "black", alpha = 0.5) + theme(legend.position = "bottom") +            
  guides(
    fill = guide_legend(
      title = "Maximal category",
      nrow = 1,
